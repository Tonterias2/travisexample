...
public interface InterestRepository extends JpaRepository<Interest, Long>, JpaSpecificationExecutor<Interest> {
    ...
    @Query(value = "select distinct interest from Interest interest left join fetch interest.communities left join fetch interest.uprofiles",
    ...
    @Query("select interest from Interest interest left join fetch interest.communities left join fetch interest.uprofiles where interest.id =:id")
