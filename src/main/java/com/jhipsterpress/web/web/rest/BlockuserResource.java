...
public class BlockuserResource {
    ...
    */
    ...
    /**
     * SEARCH  /_search/blockusers?query=:query : search for the blockuser corresponding
     * to the query.
     *
     * @param query the query of the blockuser search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/blockusers")
    @Timed
    public ResponseEntity<List<BlockuserDTO>> searchBlockusers(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Blockusers for query {}", query);
        Page<BlockuserDTO> page = blockuserService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/blockusers");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
