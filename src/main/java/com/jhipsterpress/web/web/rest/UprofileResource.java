...
public class UprofileResource {
    ...
    */
    ...
    /**
     * SEARCH  /_search/uprofiles?query=:query : search for the uprofile corresponding
     * to the query.
     *
     * @param query the query of the uprofile search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/uprofiles")
    @Timed
    public ResponseEntity<List<UprofileDTO>> searchUprofiles(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Uprofiles for query {}", query);
        Page<UprofileDTO> page = uprofileService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/uprofiles");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
