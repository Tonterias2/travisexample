...
describe('Photo e2e test', () => {
    ...
    it('should load Photos', async () => {
        ...
        expect(await photoComponentsPage.getTitle()).to.eq('jhipsterpress09App.photo.home.title');
    });

    it('should load create Photo page', async () => {
        await photoComponentsPage.clickOnCreateButton();
        photoUpdatePage = new PhotoUpdatePage();
        expect(await photoUpdatePage.getPageTitle()).to.eq('jhipsterpress09App.photo.home.createOrEditLabel');
    ...
    it('should delete last Photo', async () => {
        ...
        expect(await photoDeleteDialog.getDialogTitle()).to.eq('jhipsterpress09App.photo.delete.question');
