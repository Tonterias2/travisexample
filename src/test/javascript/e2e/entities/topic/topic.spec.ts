...
describe('Topic e2e test', () => {
    ...
    it('should load Topics', async () => {
        ...
        expect(await topicComponentsPage.getTitle()).to.eq('jhipsterpress09App.topic.home.title');
    });

    it('should load create Topic page', async () => {
        await topicComponentsPage.clickOnCreateButton();
        topicUpdatePage = new TopicUpdatePage();
        expect(await topicUpdatePage.getPageTitle()).to.eq('jhipsterpress09App.topic.home.createOrEditLabel');
    ...
    it('should delete last Topic', async () => {
        ...
        expect(await topicDeleteDialog.getDialogTitle()).to.eq('jhipsterpress09App.topic.delete.question');
